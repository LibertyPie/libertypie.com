...
async function processMediumRssFeed(data){
    ...
    if(status != "ok" || itemsArray.length == 0){ return false; }
    ...
    for(var i in itemsArray){
       ...
       var itemObj = itemsArray[i];
        ...
        if(contentHTML.length == 0) return false;
            ...
            '<div class="_news_card_item px-2" data-url="'+link+'">'+
                '<div class="item">'+
                    '<div class="card bg-light" style="height:465px;">'+
                        ...
                        '</div>'+
                        '<div class="card-footer bg-transparent border-0 text-center p-2 py-3 text-muted">Read More</div>'+
                        '<div></div>'+
                    '</div>'+
                '</div>'+
            '</div>'
